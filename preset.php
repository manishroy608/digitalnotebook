...
<head>
    ...
    <title>Reset Password-DIGITAL NOTEBOOK</title>
...
if(isset($_POST['submit'])){
	...
	$npassword=$_POST['npassword'];
	 $chk=$_SESSION['Email'];
	$query="select * from reg where Email='$chk' AND password='$cpassword'";
	$run_query=mysqli_query($conn,$query);
	if($run_query){
		if(mysqli_num_rows($run_query)>0){
			$query="UPDATE reg SET password='$npassword' WHERE Email='$chk' ";
			$run_query=mysqli_query($conn,$query);
        
        session_destroy();
 
		echo "<div class='alert alert-success'>
             <strong><b>SUCCESSFULLY !</b></strong> PASSWORD CHANGE Log in! Again.
		</div>";
		}
		else{
    ...
    <div class="col-md-4">
      <section class="login-form">
        <form method="post" action="" role="login">
          ...
          <input type="password" name="cpassword" placeholder="current password" required class="form-control input-lg" required="" />
          
          <input type="password" class="form-control input-lg" name="npassword" placeholder="New Password" required="" />
          
          
          <div class="pwstrength_viewport_progress"></div>
          
          
          <button type="submit" name="submit" class="btn btn-lg btn-danger btn-block" value="Reset Pasword">Reset Password</button>
         <!-- <div>
            <a href="regAlert.html" style="color:#41FF00">Create account</a> Or <a href="regAlert.html" style="color:#FF0008">Reset password</a>
          </div> -->
